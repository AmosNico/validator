...
@[simp]
lemma mem_empty {n} {pt : STRIPS n} {s s' : State n} : s ∈ @Path.empty _ pt s' ↔ s = s' := by
  rfl
...
lemma first_mem {n} {pt : STRIPS n} {s1 s2} (π : Path pt s1 s2) : s1 ∈ π := by
  cases π
  all_goals simp

lemma last_mem {n} {pt : STRIPS n} {s1 s2} (π : Path pt s1 s2) : s2 ∈ π := by
  induction π with
  | empty s => simp
...
lemma mem_append {n} {pt : STRIPS n} {s1 s2 s3} (π₁ : Path pt s1 s2) (π₂ : Path pt s2 s3) :
    ∀ s, s ∈ (π₁ ++ π₂) ↔ s ∈ π₁ ∨ s ∈ π₂ := by
  induction π₁ with
  | empty => simp [append, first_mem]
  | cons => grind only [hAppend_def, append, mem_cons]
...
lemma progression_monotone_states {n} {pt : STRIPS n} {A} : Monotone (pt.progression · A) := by
  intro S1 S2 hS s hs
  simp_all only [Set.le_eq_subset, mem_progression]
  obtain ⟨a, ha, s', hs', succ⟩ := hs
  use a, ha, s', hS hs'

lemma progression_monotone_actions {n} {pt : STRIPS n} {S} : Monotone (pt.progression S) := by
  intro A1 A2 hA s hs
  simp_all only [Set.le_eq_subset, mem_progression]
  obtain ⟨a, ha, s', hs', succ⟩ := hs
  use a, hA ha, s'
...
private lemma emptyCollection_def {n : ℕ} : ∅ = BitVec.zero n := rfl

@[simp]
lemma mem_empty {n i} : i ∉ (∅ : VarSet n) := by
  simp [emptyCollection_def, mem_iff]

instance {n} : Union (VarSet n) where
  union V V' := V ||| V'

lemma union_def {n : ℕ} (V V' : VarSet n) : V ∪ V' = V ||| V' := rfl

@[simp]
lemma mem_union {n} {V V' : VarSet n} {i} : i ∈ V ∪  V' ↔ i ∈ V ∨ i ∈ V' := by
  simp [union_def, mem_iff]
...
private lemma inter_def {n : ℕ} (V V' : VarSet n) : V ∩ V' = V &&& V' := rfl

@[simp]
lemma mem_inter {n} {V V' : VarSet n} {i} : i ∈ V ∩ V' ↔ i ∈ V ∧ i ∈ V' := by
  simp [inter_def, mem_iff]
...
private lemma compl_def {n : ℕ} (V : VarSet n) : Vᶜ = ~~~V := rfl

@[simp]
lemma mem_compl {n} {V : VarSet n} {i} : i ∈ Vᶜ ↔ i ∉ V := by
  simp [compl_def, mem_iff]

instance {n} : SDiff (VarSet n) where
  sdiff V V' := V &&& ~~~V'

private lemma sdiff_def {n : ℕ} (V V' : VarSet n) : V \ V' = V &&& ~~~V' := rfl

@[simp]
lemma mem_diff {n} {V V' : VarSet n} {i} : i ∈ V \ V' ↔ i ∈ V ∧ i ∉ V' := by
  simp [sdiff_def, mem_iff]
